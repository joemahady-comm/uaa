...
public class OAuth2ErrorHandler implements ResponseErrorHandler {
    ...
    public boolean hasError(ClientHttpResponse response) throws IOException {
        return HttpStatus.Series.CLIENT_ERROR.equals(HttpStatus.Series.valueOf(response.getStatusCode().value()))
                || this.errorHandler.hasError(response);
    }

    public void handleError(final ClientHttpResponse response) throws IOException {
        if (!HttpStatus.Series.CLIENT_ERROR.equals(HttpStatus.Series.valueOf(response.getStatusCode().value()))) {
