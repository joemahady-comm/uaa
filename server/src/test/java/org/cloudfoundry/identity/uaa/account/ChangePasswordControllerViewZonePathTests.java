...
/**
 * View tests for ChangePasswordController (change_password template). Each test is parameterized by
 * {@link ZoneRequestPathMode}. Asserts that the form action is zone-aware. ZONE_PATH will pass after implementation.
 */
@ExtendWith(PollutionPreventionExtension.class)
@WebAppConfiguration
@SpringJUnitConfig(classes = ChangePasswordControllerViewZonePathTests.ContextConfiguration.class)
@DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_EACH_TEST_METHOD)
class ChangePasswordControllerViewZonePathTests extends TestClassNullifier {
    ...
    private static String expectedFormAction(ZoneRequestPathMode mode) {
        String path = "/change_password.do";
    ...
    void changePasswordPageContainsZoneAwareFormAction(ZoneRequestPathMode mode) throws Exception {
    ...
    static class ContextConfiguration implements WebMvcConfigurer {
        ...
        ChangePasswordService changePasswordService() {
            return mock(ChangePasswordService.class);
        }

        @Bean
        ChangePasswordController changePasswordController(ChangePasswordService changePasswordService) {
