...
class ScimUserEndpointsMockMvcZonePathTests {
    ...
    void createUserInOtherZoneIsUnauthorized(String url, ZoneResolutionMode mode) throws Exception {
        ...
        String zoneAdminToken = mode == ZoneResolutionMode.ZONE_PATH
                ? MockMvcUtilsZonePath.getClientCredentialsOAuthAccessToken(mode, mockMvc, "admin", "admin-secret", "scim.write", subdomain, true)
                : testClient.getClientCredentialsOAuthAccessToken("admin", "admin-secret", "scim.write", subdomain);
        ...
        if (mode == ZoneResolutionMode.ZONE_PATH) {
            // With zone path, wrong-zone token may yield 302 to login instead of 401 (token not valid in resolved zone)
            resultActions.andExpect(result -> {
                int status = result.getResponse().getStatus();
                boolean unauthorized = status == 302 || status == 401;
                assertThat(unauthorized).as("Expected 401 Unauthorized or 302 redirect when using token from another zone").isTrue();
            });
        } else {
