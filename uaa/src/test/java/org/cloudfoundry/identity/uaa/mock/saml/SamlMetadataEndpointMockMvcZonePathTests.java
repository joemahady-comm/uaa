...
@DefaultTestContext
class SamlMetadataEndpointMockMvcZonePathTests {
    ...
    void get_saml_metadata_within_zone(ZoneResolutionMode mode) throws Exception {
        IdentityZone spZone = setupIdentityZone(true);
        String subdomain = spZone.getSubdomain();

        mockMvc.perform(mode.createRequestBuilder(subdomain, HttpMethod.GET, "/saml/metadata"))
                .andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CONTENT_DISPOSITION, containsString("filename=\"saml-%s-sp.xml\";".formatted(subdomain))));
    }

    @ParameterizedTest
    @EnumSource(ZoneResolutionMode.class)
    void get_saml_metadata_with_trailing_slash_within_zone(ZoneResolutionMode mode) throws Exception {
        IdentityZone spZone = setupIdentityZone(true);
        String subdomain = spZone.getSubdomain();
        // ZONE_PATH: servlet path cannot end with '/'; use /saml/metadata (same resource)
        String path = (mode == ZoneResolutionMode.ZONE_PATH) ? "/saml/metadata" : "/saml/metadata/";
        mockMvc.perform(mode.createRequestBuilder(subdomain, HttpMethod.GET, path))
                .andExpect(status().isOk())
                .andExpect(header().string(HttpHeaders.CONTENT_DISPOSITION, containsString("filename=\"saml-%s-sp.xml\";".formatted(subdomain))));
    }
    ...
    @ParameterizedTest
    @EnumSource(ZoneResolutionMode.class)
    void get_saml_metadata_extra_path_within_zone_returns_401(ZoneResolutionMode mode) throws Exception {
        IdentityZone spZone = setupIdentityZone(true);
        String subdomain = spZone.getSubdomain();

        mockMvc.perform(mode.createRequestBuilder(subdomain, HttpMethod.GET, "/saml/metadata/extra"))
