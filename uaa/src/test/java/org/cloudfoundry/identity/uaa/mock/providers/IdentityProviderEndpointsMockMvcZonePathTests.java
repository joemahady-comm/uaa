...
class IdentityProviderEndpointsMockMvcZonePathTests {
    ...
    class IdentityProvidersZonePathSupport {
        ...
        void get_identity_providers_list_responds_for_zone_path(ZoneResolutionMode mode) throws Exception {
            ...
            String token = getZoneUaaAdminToken(mode, creationResult);
            mockMvc.perform(mode.createRequestBuilder(subdomain, HttpMethod.GET, "/identity-providers")
                        .header("Authorization", "Bearer " + token)
                        .contentType(APPLICATION_JSON)
    ...
    private String getZoneUaaAdminToken(ZoneResolutionMode mode, IdentityZoneCreationResult creationResult) throws Exception {
        ...
        String token = mode == ZoneResolutionMode.ZONE_PATH
                ? MockMvcUtilsZonePath.getClientCredentialsOAuthAccessToken(mode, mockMvc, zonedClientId, zonedClientSecret, "uaa.admin", creationResult.getIdentityZone().getSubdomain(), true)
